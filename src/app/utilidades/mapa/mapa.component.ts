...
export class MapaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.capas = this.coordenadasIniciales.map(valor => {
      let marcador = marker([valor.latitud, valor.longitud], this.returnConfigIcon())
        if(valor.mensaje){
  ...
  options = {
    ...
    zoom: 14,
    center: latLng(-12.062538134306031, -77.03887939453126),
  };
  ...
  manejarClick(event: LeafletMouseEvent){
    if(!this.soloLectura){
      ...
      this.capas = [];
      this.capas.push(marker([latitud, longitud], this.returnConfigIcon()));
